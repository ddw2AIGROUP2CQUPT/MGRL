...
def get_part4_transform(type):
    ...
    if type == 'Train':
        ...
        transform_list.extend([transforms.Resize([170,170])])
    transform_list.extend(
        [transforms.ToTensor(), transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    return transforms.Compose(transform_list)
...
class SLFIR_Dataset(data.Dataset):
    def __init__(self, hp, mode):
        ...
        # self.train_condition = condition(os.path.join(self.root_dir, 'condition', 'train_condition.csv'))
        # self.test_condition = condition(os.path.join(self.root_dir, 'condition', 'test_condition.csv'))
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'Train':
            ...
            # sample = {'sketch_img': sketch_img, 'sketch_part4': sketch_part4,'sketch_part9': sketch_part9,'sketch_path': sketch_path,
            # 'positive_img': positive_img,'positive_part4':positive_part4, 'positive_part9':positive_part9,'positive_path': positive_path, 'condition': self.test_condition[positive_name],'bool_mat_4':bool_mat_4,'bool_mat_9':bool_mat_9}
...
def get_dataloader(hp):
    ...
    # # dataloader_Test = data.DataLoader(dataset_Test, batch_size=70, shuffle=False, num_workers=int(hp.nThreads))
