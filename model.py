...
class SLFIR_Model(nn.Module):
    ...
    def train_model(self, batch):
        ...
        if self.hp.condition:
            positive_feature = self.linear_network(self.cat_feature(self.attn_network(self.backbone_network(batch['positive_img'].to(self.hp.device))), batch['condition'].to(self.hp.device)))
            negative_feature = self.linear_network(self.cat_feature(self.attn_network(self.backbone_network(batch['negative_img'].to(self.hp.device))), batch['negative_condition'].to(self.hp.device)))
            sample_feature = self.linear_network(self.cat_feature(self.attn_network(self.backbone_network(batch['sketch_img'].to(self.hp.device))), batch['condition'].to(self.hp.device)))
        else:
